  ...
  IMPLICIT NONE
    ...
    SUBROUTINE Sub_real_init                        & 
               (                                    &
                ...
                z, input_nz, nz, input_Ps,          & !! added input_Ps by han
                qv_in,  temp_in, vert_in, w_in,     &
                qv_out, temp_out,  p_out, w_out       &
               )

    USE Mod_const, only: Rd, g, Cp, Ps
    IMPLICIT NONE
    ! In
    INTEGER,                   INTENT(IN)  :: read_pres, read_psfc,  & !!
                                              read_temp, input_nz,   &
                                              nz
    REAL,                      INTENT(IN)  :: input_Ps     !! added intent(in) val by han 
    ...
    if (read_pres == 1 ) then
      ...
      !!! for reading input sounding data
